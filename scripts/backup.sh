...
[ -s "$DB_BACKUP_FILE" ] || {
	printf 'ERROR: database backup is empty\n' >&2
	exit 1
}
...
if docker ps --format '{{.Names}}' | grep -qx "$APP_CONTAINER"; then
	...
	[ -n "$APP_IMAGE" ] || {
		printf 'ERROR: app container image is unavailable for offline-safe uploads backup\n' >&2
		exit 1
	}
...
[ -s "$UPLOADS_BACKUP_FILE" ] || {
	printf 'ERROR: uploads backup is empty\n' >&2
	exit 1
}
