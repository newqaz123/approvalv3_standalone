...
test.describe("Desktop UX Refresh — baseline acceptance (observed RED before production)", () => {
	...
	test("1. 1600px shell: shared-shell alignment, table visible, Created one line, no overflow", async ({
		...
		expect(cellCount, "at least one Created cell is present").toBeGreaterThan(0)
	...
	test("2. 1280px: desktop navbar visible, links on one row, secondary metadata hidden, filters unclipped", async ({
		...
		expect(navHeight, "navbar keeps a single ~64px row").toBeLessThanOrEqual(76)
		...
		expect(panelOverflow, "filter controls are unclipped at 1280px").toBeLessThanOrEqual(
			0,
		)
	})
	...
	test("3. 900px: mobile navigation visible while the desktop Requests table stays visible", async ({
		...
		await expect(mobileNav(page), "mobile navigation is visible at 900px").toBeVisible()
		await expect(desktopNavbar(page), "desktop navbar is hidden at 900px").not.toBeVisible()
	...
	test("4a. 767px: mobile navigation and mobile request cards visible; desktop navbar/table absent", async ({
		...
		await expect(mobileNav(page), "mobile navigation is visible at 767px").toBeVisible()
		await expect(
			page.locator('div[class~="md:hidden"]').locator("button").first(),
			"mobile request cards are visible at 767px",
		).toBeVisible()
		await expect(desktopNavbar(page), "desktop navbar is absent at 767px").not.toBeVisible()
		await expect(page.locator("table"), "desktop table is absent at 767px").not.toBeVisible()
	})
	...
	test("4b. 390px: mobile navigation + cards visible, desktop table absent, filter controls form one column", async ({
		...
		await expect(mobileNav(page), "mobile navigation is visible at 390px").toBeVisible()
		await expect(
			page.locator('div[class~="md:hidden"]').locator("button").first(),
			"mobile request cards are visible at 390px",
		).toBeVisible()
		await expect(page.locator("table"), "desktop table is absent at 390px").not.toBeVisible()
		...
		expect(reqBox!.y, "Requester control is stacked below Department at 390px").toBeGreaterThan(
			deptBox!.y,
		)
	})
	...
	test("5. Department/Requester controls are Radix comboboxes with listbox behavior and no Apply button", async ({
		...
		await expect(departmentCombobox, "Department trigger is a combobox").toBeVisible()
		await expect(requesterCombobox, "Requester trigger is a combobox").toBeVisible()

		// Opening the Department combobox portals a Radix listbox with options.
		await departmentCombobox.click()
		await expect(page.getByRole("listbox"), "Radix listbox opens for Department").toBeVisible()
		expect(
		...
		await expect(page.getByRole("listbox"), "Radix listbox opens for Requester").toBeVisible()
		await page.keyboard.press("Escape")
	...
	test("6. Enter and Space on a focused desktop request row open the request dialog", async ({
		...
		await expect(dialog, "Enter on a focused row opens the request dialog").toBeVisible({
			timeout: 3000,
		})
		await page.keyboard.press("Escape")
		...
		await expect(dialog, "Space on a focused row opens the request dialog").toBeVisible({
			timeout: 3000,
		})
		await page.keyboard.press("Escape")
	})
 ...
 */
const HARNESS_USERS = [
	{ name: "Ada Lovelace", email: "ada@example.com", role: "Engineering", level: "Level 1" },
	{ name: "Grace Hopper", email: "grace@example.com", role: "Production", level: "Level 2" },
	{ name: "Linus Torvalds", email: "linus@example.com", role: "Quality", level: "Level 3" },
] as const
...
async function exercisePickerFixture(page: Page, label: string) {
	...
	for (const [field, query, expectedText] of matches) {
		...
		await expect(
			openCommand(page).locator("[cmdk-item]").filter({ hasText: expectedText }),
	...
	await page.locator(`[data-picker-fixture="${label}"] [data-picker-exhaust]`).click()
	await fixtureOpen(page, label).click()
	await expect(
	...
	await page.keyboard.press("Escape")
	await page.locator(`[data-picker-fixture="${label}"] [data-picker-reset]`).click()
}
