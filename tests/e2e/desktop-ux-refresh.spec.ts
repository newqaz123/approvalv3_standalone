...
const filtersPanel = (page: Page) =>
  page.locator('div.rounded-lg.border.bg-gray-50').filter({ hasText: 'Filters' })
...
test.describe('Desktop UX Refresh — baseline acceptance (observed RED before production)', () => {
  ...
  test('1. 1600px shell: navbar/main edges align, table visible, Created one line, no overflow', async ({
    ...
    for (let i = 0; i < Math.min(cellCount, 5); i++) {
      ...
      expect(innerHeight, `Created cell ${i} stays on one line`).toBeLessThanOrEqual(28)
    }
  ...
  test('2. 1280px: desktop navbar visible, links do not wrap, secondary metadata hidden, filters unclipped', async ({
    ...
    for (const label of ['Requests', 'My Actions', 'Analytics', 'Budget Monitor', 'Admin Panel']) {
      await expect(
    ...
    expect(panelOverflow, 'filter controls are unclipped at 1280px').toBeLessThanOrEqual(0)
  })
  ...
  test('3. 900px: mobile navigation visible while the desktop Requests table stays visible', async ({
    ...
    await expect(page.locator('table').first(), 'desktop Requests table is visible at 900px').toBeVisible()
    await expect(
  ...
  test('5. Department/Requester controls expose Radix listbox behavior with no Apply button', async ({
    ...
    expect(optionCount, 'Department listbox exposes selectable options').toBeGreaterThan(0)
  ...
  test('7. /test-harness/hierarchy-pickers exposes five real picker fixtures with full search behavior', async ({
    ...
    await expect(fixtures, 'harness exposes five labeled real picker fixtures').toHaveCount(5)

    const labels = ['solution', 'submitter', 'submit-final', 'final-resubmit', 'custom']
    for (const label of labels) {
      ...
      await search.fill('')
      await fixture.getByRole('button', { name: /preselect all|select all/i }).click()
      await expect(
