...
it('uses configured production origins and a relative logout callback', () => {
  // Sign-out lives in the shared UserMenu rendered by the navbar and mobile shell.
  const userMenu = readFileSync('src/components/navigation/user-menu.tsx', 'utf8')
  const env = readFileSync('.env.example', 'utf8')
  assert.match(userMenu, /signOut\(\{ callbackUrl: ["']\/sign-in["'] \}\)/)
  assert.doesNotMatch(userMenu, /localhost:3000/)
