...
describe("Gapforimprove regressions", () => {
	...
	it("engineering dashboard sends plain request objects to client tabs", () => {
		...
		assert.doesNotMatch(
			requests,
			/request: \{\s*\.\.\.request,\s*hasRejection/,
		);
	...
	it("notification refresh does not replace an open notification list with loading state", () => {
		...
		assert.match(
			bell,
			/if \(!hasLoadedNotifications\) \{\s*setIsLoading\(true\)\s*\}/,
		);
	...
	it("request list refreshes local table state after mutations and router refreshes", () => {
		...
		assert.match(
			listWithFilters,
			/useEffect\(\(\) => \{\s*setRequests\(initialRequests\)\s*\}, \[initialRequests\]\)/,
		);
	...
	it("request mutations invalidate every request-backed page surface", () => {
		const invalidation = read(
			"src/server-actions/request-view-invalidation.ts",
		);
		const requests = read("src/server-actions/requests.ts");
	...
	it("email notification request links open the same modal flow as in-app notifications", () => {
		...
		const deepLinkModal = read(
			"src/components/requests/request-deep-link-modal.tsx",
		);
		const myActionsPage = read(
	...
	it("approval and rejection emails include request details and use the email helper", () => {
		...
		const notifyNextApprover =
			approvals.match(/async function notifyNextApprover[\s\S]*?\n\}/)?.[0] ??
			"";
		...
		const directNotificationCreates =
			`${approvals}\n${solutions}`.match(
				/notifications\.create(?:Many)?\(\{[\s\S]*?\n\s*\}\)/g,
			) ?? [];

		for (const createCall of directNotificationCreates) {
			assert.doesNotMatch(
				createCall,
				/approval_needed|final_approval_needed|approval_rejected/,
			);
		}
	});
	...
	it("sends solution submission emails after the solution transaction commits", () => {
		...
		const submitSolution =
			solutions.match(
				/export async function submitSolution[\s\S]*?\n\}/,
			)?.[0] ?? "";
		const transactionStart = submitSolution.indexOf(
			"const result = await prisma.$transaction",
		);
		const notificationSendStart = submitSolution.indexOf(
		...
		const transactionBlock = submitSolution.slice(
			transactionStart,
		...
		assert.doesNotMatch(
			transactionBlock,
			/createNotification|notifyUsersInDepartment|tx\.notifications\.create/,
		);
		assert.match(
			submitSolution,
			/await Promise\.all\(\s*pendingNotifications\.map\(\(notification\) =>\s*createNotification\(notification\)/,
		);
	});

	it("merges department-wide notification emails while keeping per-user in-app notifications", () => {
		const notifications = read("src/server-actions/notifications.ts");
		const notifyUsersInDepartment =
			notifications.match(
				/export async function notifyUsersInDepartment[\s\S]*?^}/m,
			)?.[0] ?? "";

		assert.match(
			notifyUsersInDepartment,
			/select:\s*\{\s*id:\s*true,\s*email:\s*true\s*\}/,
		);
		...
		assert.match(
			notifyUsersInDepartment,
			/sendEmailNotification\(\s*notification,\s*emailRecipients/,
		);
