...
describe("engineering sub-task schema", () => {
	...
	const migration = () =>
		readFileSync(
			"prisma/migrations/20260606000000_add_engineering_sub_tasks/migration.sql",
			"utf8",
		);
	...
	it("adds request sub-task, stage, and subcontractor models", () => {
		...
		assert.match(
			source,
			/workRequisitionReceived\s+Boolean\s+@default\(false\)/,
		);
...
describe("engineering sub-task helpers", () => {
	it("limits sub-task visibility to the approved request stages", () => {
		...
		assert.equal(
			isSubTaskVisibleForRequestStatus("DesignCostEstimationApproval"),
			false,
		);
	...
	it("filters rows by WR state", () => {
		...
		assert.deepEqual(
			filterRowsByWorkRequisition(rows, "all").map((row) => row.id),
			["a", "b"],
		);
		assert.deepEqual(
			filterRowsByWorkRequisition(rows, "received").map((row) => row.id),
			["a"],
		);
		assert.deepEqual(
			filterRowsByWorkRequisition(rows, "not-received").map((row) => row.id),
			["b"],
		);
	});
	...
	it("allows only engineering and admin users to manage sub-tasks", () => {
		...
		assert.equal(
			canManageEngineeringSubTasks({ role: "engineering", isActive: false }),
			false,
		);
	...
	it("detects stale incomplete sub-tasks by updatedAt and stage", () => {
		...
		assert.equal(
			isSubTaskStale(oldWaitingQuotation, { olderThanDays: 7, now }),
			true,
		);
		assert.equal(
			isSubTaskStale(recentWaitingQuotation, { olderThanDays: 7, now }),
			false,
		);
		assert.equal(
			isSubTaskStale(completedOldTask, { olderThanDays: 7, now }),
			false,
		);
		assert.equal(
			isSubTaskStale(oldWaitingQuotation, {
				olderThanDays: 7,
				stageId: "other-stage",
				now,
			}),
	...
	it("requires custom stage text only for Others", () => {
		assert.deepEqual(
			validateSubTaskInput({ description: "", stageIsOthers: false }),
			{
				success: false,
...
describe("engineering sub-task server action wiring", () => {
	it("exposes CRUD, WR, stages, subcontractor, and stale filter actions", () => {
		const source = readFileSync(
			"src/server-actions/engineering-sub-tasks.ts",
			"utf8",
		);
	...
	it("exports exactly the planned server actions", () => {
		...
		const exportedAsyncFunctions = Array.from(
			source.matchAll(/export async function (\w+)\b/g),
			(match) => match[1],
		).sort();
	...
	it("moves checked sub-tasks to the Completed stage and records the editor", () => {
		...
		const completionBlock =
			source.match(
				/export async function setSubTaskCompleted[\s\S]*?export async function deleteSubTask/,
			)?.[0] ?? "";

		assert.match(
			completionBlock,
			/prisma\.sub_task_stages\.findFirst\(\{[\s\S]*name:\s*'Completed'/,
		);
	...
	it("rejects inactive users, deleted requests, and inactive subcontractors before mutations", () => {
		...
		assert.match(
			source,
			/select:\s*\{\s*id:\s*true,\s*status:\s*true,\s*isDeleted:\s*true\s*\}/,
		);
		assert.match(
			source,
			/if \(!request\s*\|\|\s*request\.isDeleted\) throw new Error\('Request not found'\)/,
		);
		assert.match(
			source,
		...
		assert.match(
			source,
			/prisma\.sub_contractors\.findUnique\(\{[\s\S]*where:\s*\{\s*id: subContractorId\s*\}/,
		);
		assert.match(
			source,
...
describe("engineering sub-task request and filter wiring", () => {
	it("adds WR fields to request server rows and request detail data", () => {
		...
		assert.match(
			requests,
			/orderBy:\s*\[\s*\{\s*isCompleted: 'asc'\s*\},\s*\{\s*updatedAt: 'desc'\s*\}/,
		);
	});
	...
	it("wires WR filters into requests API, request filters, and dashboard filters", () => {
		...
		const requestFilters = readFileSync(
		...
		const dashboardTable = readFileSync(
			"src/components/dashboard/dashboard-table.tsx",
			"utf8",
		);
		const dashboardTabs = readFileSync(
...
describe("engineering sub-task request modal panel wiring", () => {
	...
	it("adds the collapsed Sub-tasks section with WR and task management actions", () => {
		const source = readFileSync(
			"src/components/requests/sub-tasks-section.tsx",
			"utf8",
		);
		const taskCardBlock =
			source.match(/data-testid="sub-task-card"[\s\S]*?\)\s*\}\)\}/)?.[0] ?? "";

		assert.match(source, /'use client'/);
		assert.match(source, /const \[isOpen, setIsOpen\] = useState\(false\)/);
		assert.match(
			source,
			/Collapsible[\s\S]*open=\{isOpen\}[\s\S]*onOpenChange=\{setIsOpen\}/,
		);
		...
		const collapsedTrigger =
			source.match(/<CollapsibleTrigger[\s\S]*?<\/CollapsibleTrigger>/)?.[0] ??
			"";
		assert.match(
			collapsedTrigger,
			/<Badge[\s\S]*\{summary\.label\}[\s\S]*<\/Badge>/,
		);
	...
	it("adds an add/edit sub-task form dialog with Others stage custom text", () => {
		const source = readFileSync(
			"src/components/requests/sub-task-form-dialog.tsx",
			"utf8",
		);
		...
		assert.match(
			source,
			/task \? \(\s*<span className="sr-only">Edit sub-task<\/span>/,
		);
	...
	it("routes visible requests to SubTasksSection with options and permissions", () => {
		...
		const subTaskElementBlock =
			source.match(/const subTasksElement =[\s\S]*?\) : undefined/)?.[0] ?? "";
	...
	it("renders the optional sub-task section before each activity timeline", () => {
		for (const file of modalFiles) {
			...
			assert.match(
				source,
				/\{subTasksElement && \(\s*<>\s*<Separator[\s\S]*?\{subTasksElement\}/,
			);
...
describe("engineering stale sub-task filters", () => {
	...
	it("renders live follow-up work filters and sub-task progress metadata", () => {
		...
		assert.doesNotMatch(
			tabs,
			/<h2 className="text-lg font-semibold text-gray-900">Follow up work<\/h2>/,
		);
	...
	it("keeps completed requests out of follow-up work and improves engineering summary cards", () => {
		...
		assert.match(
			page,
			/subTasks:\s*\{\s*some:\s*\{\s*isCompleted:\s*false\s*\}/,
		);
		assert.match(
			page,
...
describe("admin sub-task stage management wiring", () => {
	it("adds admin-managed global stage actions", () => {
		const actions = readFileSync(
			"src/server-actions/sub-task-stages.ts",
			"utf8",
		);
	...
	it("renders stage settings on a separate admin page with cards, menu actions, and Completed-only default badge", () => {
		const component = readFileSync(
			"src/components/admin/sub-task-stage-settings.tsx",
			"utf8",
		);
		const page = readFileSync("src/app/admin/page.tsx", "utf8");
		const stagePage = readFileSync(
			"src/app/admin/sub-task-stages/page.tsx",
			"utf8",
		);
	...
	it("adds inline subcontractor creation in the sub-task form dropdown", () => {
		...
		assert.match(
			actions,
			/select:\s*\{\s*id:\s*true,\s*name:\s*true,\s*isActive:\s*true\s*\}/,
		);
	});
});
