...
`);
	...
	assert.equal(
		parsed.DATABASE_URL,
		"postgresql://postgres:changeme@db:5432/app_db?schema=public",
	);
...
test("createOriginReport rejects localhost and conflicting production origins", () => {
	...
	assert.equal(
		report.issues.some((issue) => issue.includes("localhost")),
		true,
	);
	assert.equal(
		report.issues.some((issue) => issue.includes("same origin")),
		true,
	);
	assert.equal(
		report.issues.some((issue) => issue.includes("AUTH_TRUST_HOST=true")),
		true,
	);
});
...
`);
	...
	assert.equal(
		report.originIssues.some((issue) => issue.includes("localhost")),
		true,
	);
	assert.equal(
		report.originIssues.some((issue) => issue.includes("same origin")),
		true,
	);
	assert.equal(
		report.originIssues.some((issue) => issue.includes("AUTH_TRUST_HOST=true")),
		true,
	);
});
...
test("createRuntimeReport accepts the Docker production contract", () => {
	const report = createRuntimeReport({
		DATABASE_URL:
			"postgresql://app-user:strong-password@db:5432/app_db?schema=public",
...
test("env-check CLI exits 1 on runtime issues and never prints secret values", async () => {
	...
	const result = spawnSync(
		process.execPath,
		["tools/env-check.mjs", "--env", envPath, "--template", templatePath],
