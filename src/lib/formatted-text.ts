  ...
  while (index < source.length) {
    ...
    if (source.startsWith('**', index)) {
      ...
      if (openerFollowedByContent) {
        ...
        if (closeIndex !== -1) {
          appendStyledContent(tokens, 'bold', source.slice(contentStart, closeIndex))
          index = closeIndex + 2
          continue
        }
      }
...
export function truncateFormattedText(
  ...
  const fullVisibleLength = tokens.reduce((sum, token) => sum + tokenVisibleLength(token), 0)
...
function selectTokens(source: string, maxVisibleCharacters?: number): FormattedTextToken[] {
