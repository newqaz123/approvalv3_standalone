...
import { buildFollowUpDashboard } from "@/lib/follow-up-dashboard";
export type {
	FollowUpActivity,
...
export async function getPendingMyApprovals(): Promise<RequestListRow[]> {
	...
	const solutionOrConditions: any[] = [{ requiredApproverId: user.id }];
		...
		if (blockingApprovals === 0) {
			...
			if (!seenRequestIds.has(approval.request.id)) {
				...
				const approvals = await Promise.all(
					approval.request.approvals.map(async (a) => {
						if (
							a.status === "pending" &&
							!a.requiredApprover &&
							approval.request.departmentId
						) {
							try {
								const potentialApprovers = await getApproversAtLevel(
									approval.request.departmentId,
									a.requiredLevel,
								);
								return {
									...a,
									potentialApprovers: potentialApprovers.map((p) => ({
										name: p.name,
									})),
								};
							} catch (error) {
								console.error(
									"[getPendingMyApprovals] Error loading potential approvers:",
									error,
								);
								return a;
							}
	...
	for (const approval of pendingSolutionApprovals) {
		...
		if (blockingSolutionApprovals === 0) {
			...
			const hasRejection =
				(await prisma.solution_approvals.count({
					where: {
						solutionId: approval.solutionId,
						status: "rejected",
					},
...
export async function getMyCreatedRequests(): Promise<RequestListRow[]> {
	...
	const processedRequests = await Promise.all(
		requests.map(async (req) => {
			...
			const useSolutionApprovals =
				req.status === "DesignCostEstimationApproval" &&
				req.solutions.length > 0;

			// Map solution approvals to request approval format
			const solutionApprovals =
				useSolutionApprovals && req.solutions[0]?.approvals
					? req.solutions[0].approvals.map((sa) => ({
							id: sa.id,
							status: sa.status,
							approver: sa.approver ? { name: sa.approver.name } : null,
							requiredApprover: sa.requiredApprover
								? { name: sa.requiredApprover.name }
								: null,
			...
			const rawApprovals = useSolutionApprovals
				? solutionApprovals
				: req.approvals;
			...
			const approvalDepartmentId =
				useSolutionApprovals && req.solutions[0]?.submittedBy?.departmentId
				...
				rawApprovals.map(async (approval) => {
					...
					if (
						...
						try {
							...
							console.error(
								"[getMyCreatedRequests] Error loading potential approvers:",
								error,
							);
							return approval;
						}
				...
				hasRejection:
					req.solutions.some(
						(s) =>
							...
							s.approvals.some((a: any) => a.status === "rejected"),
				...
				rawApprovals.map(async (approval) => {
					...
					if (
						...
						try {
							...
							console.error(
								"[getAllRequests] Error loading potential approvers:",
								error,
							);
							return approval;
						}
...
export async function getFollowUpDashboard() {
	...
	const visible = await getMyRequests({ wrStatus: "all" });
	const sendBackIds = visible
		.filter((request) => request.status === "SendBackToRequester")
		.map((request) => request.id);

	const solutions = sendBackIds.length
		? await prisma.solutions.findMany({
				where: { requestId: { in: sendBackIds } },
		...
		: [];
	const estimateByRequest = new Map<string, string>();
	for (const solution of solutions) {
		if (estimateByRequest.has(solution.requestId)) continue;
		estimateByRequest.set(
			solution.requestId,
			`Estimate ${solution.currency === "THB" ? "฿" : `${solution.currency} `}${Number(solution.costEstimate).toLocaleString()}`,
		);
	}

	const visibleIds = visible.map((request) => request.id);
	const [recentActivities, statusActivities] = visibleIds.length
		? await Promise.all([
				prisma.request_activities.findMany({
					where: { requestId: { in: visibleIds } },
					orderBy: { createdAt: "desc" },
					take: 20,
					select: {
						id: true,
				...
				prisma.request_activities.findMany({
					where: {
						requestId: { in: visibleIds },
						OR: [{ fromStatus: { not: null } }, { toStatus: { not: null } }],
					},
	...
	return buildFollowUpDashboard({
		visible: visible.map((request) => ({
			...
			workRequisitionReceived: request.workRequisitionReceived,
			estimateLabel: estimateByRequest.get(request.id) ?? null,
			approvals: request.approvals,
			engineerAssignments: request.engineerAssignments,
		})),
		activities: statusActivities.flatMap((activity) =>
			activity.requestId
				? [
						{
		...
		recentActivity: recentActivities.map((activity) => ({
			...
			title: activity.request?.title || "Request",
			actorName: activity.user?.name || "Someone",
		})),
