...
function buildNotificationRequestPath(
	notification: NotificationRequestLinkInput,
) {
	...
	if (
		...
		notification.type === "final_approval_needed"
	) {
...
async function getNotificationRequestDetails(
	requestId?: string,
): Promise<RequestEmailDetails | null> {
	if (!requestId) {
		return null;
	}
	...
	return {
		...
		projectEstimateCost:
			request.projectEstimateCost === null
				? null
				: Number(request.projectEstimateCost),
		engineeringCostEstimate:
			request.engineeringCostEstimate === null
				? null
				: Number(request.engineeringCostEstimate),
		latestSolutionEstimate: latestSolution
			? Number(latestSolution.costEstimate)
			: null,
...
function buildFormattedDescriptionRow(safeHtml: string) {
	// safeHtml is trusted output-only email presentation: approved palette marks
	// are materialized and inline images are URL-free escaped alt placeholders.
	return `
    <tr>
...
function resolveEmailCostEstimate(details: RequestEmailDetails) {
	if (details.latestSolutionEstimate !== null) {
		return formatMoney(
			details.latestSolutionEstimate,
			details.latestSolutionCurrency ?? "THB",
		);
	}
...
export async function getNotifications(userId: string, limit: number = 20) {
	...
	const currentUserId = _authUser?.id;
...
export async function notifyUsersInDepartment(
	...
	if (emailRecipients.length > 0) {
		await sendEmailNotification(notification, emailRecipients).catch(
			(error) => {
				// Log error but don't fail the notification creation
...
async function sendEmailNotification(
	...
	const requestDetails = await getNotificationRequestDetails(
