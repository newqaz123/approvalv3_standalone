...
export function wrapSelectionWithBold(
	...
	const start = Math.max(
		0,
		Math.min(selectionStart, selectionEnd, value.length),
	);
	const end = Math.max(
		0,
		Math.min(Math.max(selectionStart, selectionEnd), value.length),
	);
	...
	let leadingEnd = 0;
	while (
		leadingEnd < selected.length &&
		isSelectionWhitespace(selected[leadingEnd]!)
	) {
		leadingEnd += 1;
	}
	...
	while (
		...
		isSelectionWhitespace(selected[trailingStart - 1]!)
	) {
		trailingStart -= 1;
	}
...
export const FormattedTextarea = React.forwardRef<
	HTMLTextAreaElement,
