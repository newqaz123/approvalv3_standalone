...
import { FileText, ChevronRight, Clock, UserCircle, Check } from "lucide-react";
	...
	return (
		...
		>
				...
				<h3 className="font-medium text-gray-900 flex-1 line-clamp-2">
					...
					{request.hasRejection &&
						(request.status === "ImprovementRequest" ||
							request.status === "SentToEngineer") && (
							<span className="ml-2 inline-flex items-center">
								<RejectedBadge size="sm" showText={false} />
							</span>
						)}
					{request.workRequisitionReceived && (
						<span className="ml-1.5 inline-flex shrink-0 items-center gap-0.5 rounded-full bg-sky-50 px-1.5 py-0.5 align-middle text-[10px] font-bold uppercase tracking-wide text-sky-700 ring-1 ring-inset ring-sky-600/25">
			...
			{(showRequester ||
				...
				(request.engineerAssignments &&
					request.engineerAssignments.length > 0)) && (
				<div className="flex flex-wrap items-center gap-3 text-xs text-gray-600 border-t border-gray-100 pt-2 mt-1">
					...
					{showApprovalProgress &&
						request.approvalLevel !== undefined &&
						request.totalApprovalLevels !== undefined && (
							<div className="flex items-center gap-1">
					...
					{showApprovalProgress &&
						...
						request.totalApprovals !== undefined && (
							<div className="flex items-center gap-1">
