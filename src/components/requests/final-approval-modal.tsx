...
function getFileIcon(fileType: string) {
	switch (fileType.toLowerCase()) {
		case "pdf":
			return (
				<div className="shrink-0 size-8 rounded bg-red-50 dark:bg-red-900/20 flex items-center justify-center text-red-600">
					<FileText className="w-4 h-4" />
				</div>
			);
		case "image":
			return (
				<div className="shrink-0 size-8 rounded bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center text-blue-600">
					<FileImage className="w-4 h-4" />
				</div>
			);
		case "docx":
			return (
				<div className="shrink-0 size-8 rounded bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-600 dark:text-slate-400">
					<File className="w-4 h-4" />
				</div>
			);
		case "xlsx":
			return (
				<div className="shrink-0 size-8 rounded bg-green-50 dark:bg-green-900/20 flex items-center justify-center text-green-600">
...
function RetractableActivityTimeline({
	activities,
}: {
	activities: ActivityItem[];
}) {
...
function InitiateFinalApproval({
	...
	const availableUsersFiltered = availableUsers.filter(
		(u) => !customApprovers.includes(u.id),
	);
	...
	return (
		<div className="bg-amber-50/80 dark:bg-amber-900/10 rounded-xl border border-amber-200 dark:border-amber-800/30 p-4">
			...
			<p className="text-xs text-amber-700 dark:text-amber-300 mb-4">
				Route this solution through the final department approval chain. Choose
				to use the default hierarchy or set a custom approval chain.
			</p>
			...
			<div className="flex gap-2 mb-4">
				...
				>
					...
					<div className="text-xs text-slate-500 mt-1">
						Use configured department levels
					</div>
				</button>
				<button
							...
							{selectedUsers.map((user, index) => (
								...
								>
									...
									<button
										onClick={() =>
											setCustomApprovers(
												customApprovers.filter((id) => id !== user.id),
											)
										}
			...
			<Button
				onClick={() =>
					onInitiate?.(useCustom, useCustom ? customApprovers : undefined)
				}
				...
				<div className="min-h-0 flex-1 overflow-y-auto p-6 space-y-6 max-h-[calc(92svh-140px)] pointer-fine:max-h-[calc(90vh-140px)]">
							...
							<div className="flex items-start gap-3">
								...
								<div className="flex-1">
									...
									<p className="text-xs text-red-600 dark:text-red-300 mt-1">
										Rejected by {data.rejection.rejectedBy} on{" "}
										{format(new Date(data.rejection.rejectedAt), "MMM d, yyyy")}
									</p>
					...
					<section className="bg-amber-50/60 dark:bg-amber-900/10 rounded-xl p-5 border border-amber-200/60 dark:border-amber-800/30">
						...
						<div className="flex items-center justify-between text-xs text-slate-500">
							...
							<span>
								{format(new Date(data.solution.submittedAt), "MMM d, yyyy")}
							</span>
						</div>
					</section>
							...
							<div className="grid grid-cols-2 gap-4 text-sm">
								<div>
									...
									<p className="font-medium">
										{data.finalApproval.initiatedBy}
									</p>
								</div>
								<div>
									...
									<p className="font-medium">
										{format(
											new Date(data.finalApproval.initiatedAt),
											"MMM d, yyyy",
										)}
									</p>
								</div>
								<div>
									...
									<p className="font-medium">
										Level {data.finalApproval.currentLevel} of{" "}
										{data.finalApproval.totalLevels}
									</p>
								</div>
