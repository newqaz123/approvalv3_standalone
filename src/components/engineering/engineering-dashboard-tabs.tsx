...
export function EngineeringDashboardTabs({
	...
	const [activeTab, setActiveTab] = useState<"needs-action" | "all">(
		"needs-action",
	);
	...
	return (
		<div className="space-y-6">
			...
			{activeTab === "needs-action" ? (
				<>
					{/* Needs Action List */}
					<NeedsActionList needsApproval={needsApproval} />
				</>
			) : (
				<AllEngineeringRequests
...
function AllEngineeringRequests({
	...
	const [selectedSolutionRequestId, setSelectedSolutionRequestId] = useState<
		string | null
	>(null);
	...
	const [sentToEngineerVisibleCount, setSentToEngineerVisibleCount] =
		useState(FOLLOW_UP_PAGE_SIZE);
	const [followUpVisibleCount, setFollowUpVisibleCount] =
		useState(FOLLOW_UP_PAGE_SIZE);

	useEffect(() => {
		setSentToEngineerVisibleCount(FOLLOW_UP_PAGE_SIZE);
		setFollowUpVisibleCount(FOLLOW_UP_PAGE_SIZE);
	}, [selectedStage, subContractorIds, olderThanDays, selectedEngineer]);

	const sortedSubContractors = useMemo(
		() =>
			[...subContractors].sort((left, right) =>
	...
	const filteredSubContractors = useMemo(
		() =>
			sortedSubContractors.filter((subContractor) =>
				fuzzyMatch(subContractor.name, subContractorQuery),
			),
		[sortedSubContractors, subContractorQuery],
	);

	const selectedSubContractorIdSet = useMemo(
		() => new Set(subContractorIds),
		[subContractorIds],
	);
	const selectedSubContractors = sortedSubContractors.filter((subContractor) =>
		selectedSubContractorIdSet.has(subContractor.id),
	);
	const selectedStalePreset = STALE_DAY_PRESETS.find(
		(preset) => preset === olderThanDays,
	);
	const visibleSubContractorChips =
		subContractorIds.length >= 3 ? [] : selectedSubContractors;
	const collapsedSubContractorChip =
		subContractorIds.length >= 3
			? `${selectedSubContractors.length} subcontractors`
			: null;
	const subContractorButtonLabel =
	...
	const filteredRequests = requests.filter((request) => {
		...
		const matchesEngineer =
			...
			(selectedEngineer === ENGINEER_UNASSIGNED_FILTER
				...
				: request.assignedEngineers.some(
		...
		return request.subTasks.some((subTask) => {
			const matchesStage =
				!hasStageFilter || subTask.stage.id === selectedStage;
			const matchesSubContractor =
				!hasSubContractorFilter ||
				(!!subTask.subContractor?.id &&
					selectedSubContractorIdSet.has(subTask.subContractor.id));
			const matchesOlderThanDays =
				!threshold ||
				(!subTask.isCompleted && new Date(subTask.updatedAt) <= threshold);

			return matchesStage && matchesSubContractor && matchesOlderThanDays;
		});
	});

	const sentToEngineerRequests = filteredRequests.filter(
		(request) => request.status === "SentToEngineer",
	);
	const followUpRequests = filteredRequests.filter(
		(request) => request.status !== "SentToEngineer",
	);
	const visibleSentToEngineer = sentToEngineerRequests.slice(
		0,
		sentToEngineerVisibleCount,
	);
	const visibleFollowUp = followUpRequests.slice(0, followUpVisibleCount);
	const sentToEngineerHiddenCount = Math.max(
		sentToEngineerRequests.length - visibleSentToEngineer.length,
		0,
	);
	const followUpHiddenCount = Math.max(
		followUpRequests.length - visibleFollowUp.length,
		0,
	);
	...
	const handleSubContractorPillClick = (
		...
		setSubContractorIds((current) =>
			current.includes(subContractor.id)
				? current
				: [...current, subContractor.id],
		);
		setSubContractorQuery("");
	};
	...
	return (
		<>
			<div className="rounded-lg border bg-gray-50 px-4 py-3">
				<div
					data-testid="follow-up-filter-chipline"
				...
				>
					...
					<span className="mx-1 text-sm font-semibold text-gray-500">
						Sub-contractor
					</span>
					{visibleSubContractorChips.map((subContractor) => (
						<Badge
							key={subContractor.id}
							className="gap-1 rounded-full border border-blue-200 bg-blue-50 px-2.5 py-1 text-xs text-blue-700 hover:bg-blue-50"
						>
							{subContractor.name}
							<button
								type="button"
					...
					<Popover
						open={subContractorFilterOpen}
						onOpenChange={setSubContractorFilterOpen}
					>
								...
								<CommandList>
									<CommandGroup>
										{filteredSubContractors.map((subContractor) => (
											...
											>
												<Checkbox
													checked={selectedSubContractorIdSet.has(
														subContractor.id,
													)}
			...
			<Card>
				<CardContent className="pt-6">
					<p className="mb-4 flex items-center gap-2 rounded-md border border-blue-100 bg-blue-50 px-3 py-2 text-sm text-blue-800">
						...
						<span>
							This table does not show completed requests, including work with
							WR received and every sub-task checked complete, or cancelled
							work.
						</span>
					</p>
					{filteredRequests.length === 0 ? (
						...
						<div className="space-y-8">
							<section className="space-y-3">
								<div className="flex items-end justify-between gap-3">
									<div>
										...
										<p className="text-sm text-gray-500">
											Needs an engineering solution before approval can
											continue.
										</p>
									</div>
									<span className="rounded-full border border-gray-200 px-2.5 py-1 text-xs font-semibold text-gray-500">
										{sentToEngineerRequests.length} open
									</span>
								</div>
								{visibleSentToEngineer.length === 0 ? (
									<div className="rounded-lg border border-dashed py-6 text-center text-sm text-gray-500">
										No requests awaiting a solution
									</div>
								) : (
									<div className="space-y-2">
										{visibleSentToEngineer.map((request) => (
											<FollowUpRequestRow
												key={request.id}
												request={request}
												engineeringUsers={engineeringUsers}
												...
												onKeyDown={handleRequestRowKeyDown}
												onSubmitSolution={handleSubmitSolution}
												onSubContractorPillClick={handleSubContractorPillClick}
								...
								{sentToEngineerHiddenCount > 0 && (
									<Button
										type="button"
										variant="outline"
										className="h-10 w-full border-dashed text-sm font-semibold"
										onClick={() =>
											setSentToEngineerVisibleCount(
												(current) => current + FOLLOW_UP_PAGE_SIZE,
											)
										}
									>
							...
							<section className="space-y-3">
								<div className="flex items-end justify-between gap-3">
									<div>
										...
										<p className="text-sm text-gray-500">
											Sent back or final approval is still open because WR or a
											sub-task is incomplete.
										</p>
									</div>
									<span className="rounded-full border border-gray-200 px-2.5 py-1 text-xs font-semibold text-gray-500">
										{followUpRequests.length} open
									</span>
								</div>
								...
								{followUpHiddenCount > 0 && (
									<Button
										type="button"
										variant="outline"
										className="h-10 w-full border-dashed text-sm font-semibold"
										onClick={() =>
											setFollowUpVisibleCount(
												(current) => current + FOLLOW_UP_PAGE_SIZE,
											)
										}
									>
										Show {followUpHiddenCount} more in Follow-up work
									</Button>
								)}
							</section>
						</div>
...
function FollowUpRequestRow({
	request,
	engineeringUsers,
	currentUserId,
	showSubmitSolution = false,
	onOpen,
	onKeyDown,
	onSubmitSolution,
	onSubContractorPillClick,
	formatCreatedDate,
	formatLastUpdate,
}: {
	...
	currentUserId: string;
	showSubmitSolution?: boolean;
	onOpen: (requestId: string) => void;
	onKeyDown: (event: KeyboardEvent<HTMLDivElement>, requestId: string) => void;
	onSubmitSolution: (requestId: string) => void;
	onSubContractorPillClick: (
		event: React.MouseEvent<HTMLButtonElement>,
		name: string,
	) => void;
	formatCreatedDate: (value: Date | string) => string;
	...
	const visibleSubContractorNames = subContractorNames.slice(0, 2);
	const hiddenSubContractorCount = Math.max(subContractorNames.length - 2, 0);
	...
	return (
		<div
			...
			onKeyDown={(event) => onKeyDown(event, request.id)}
			className="w-full cursor-pointer rounded-2xl border border-gray-200 bg-white px-4 py-4 text-left transition-colors hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
		>
			<div
			...
			>
				<div className="min-w-0 flex-1">
					...
					<p
						data-testid="follow-up-request-meta"
				...
				<div className="flex shrink-0 items-center gap-2">
					<div onClick={(event) => event.stopPropagation()}>
						<EngineerPicPicker
							...
							engineeringUsers={engineeringUsers}
							initialAssignedIds={request.assignedEngineers.map(
			...
			<HoverCard>
				<HoverCardTrigger asChild>
					...
					>
						<Progress
							value={progressValue}
							className="h-1 w-full bg-slate-200 transition-all duration-200 motion-safe:group-hover/progress:h-1.5 [&>div]:bg-emerald-500 [&>div]:transition-all [&>div]:duration-200 motion-safe:group-hover/progress:[&>div]:bg-emerald-400"
						/>
			...
			{subContractorNames.length > 0 && (
				...
				>
					{visibleSubContractorNames.map((name) => (
						<button
							key={name}
							type="button"
							onClick={(event) => onSubContractorPillClick(event, name)}
							className="rounded-full border border-gray-200 bg-white px-2.5 py-1 text-xs font-medium text-gray-600 hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700"
						>
							{name}
						</button>
					))}
					{hiddenSubContractorCount > 0 && (
						<span className="rounded-full border border-gray-200 px-2.5 py-1 text-xs font-medium text-gray-500">
							+{hiddenSubContractorCount} more
						</span>
					)}
				</div>
			)}
...
function FilterSeparator() {
	return (
		<span
			className="mx-3 hidden h-8 w-px bg-gray-300 sm:inline-block"
			aria-hidden="true"
		/>
	);
}
