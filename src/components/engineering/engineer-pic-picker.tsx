		...
		<Popover open={open} onOpenChange={setOpen}>
			<PopoverTrigger asChild>
				<Button
					...
					size="sm"
					className={cn(
						"h-auto min-h-[36px] justify-start gap-2 transition-all duration-200 motion-safe:hover:-translate-y-0.5 motion-safe:hover:scale-[1.02] motion-safe:hover:shadow-sm motion-reduce:transform-none",
						selectedUsers.length === 0 &&
							"border-[#fecaca] bg-[#fef2f2] text-[#b91c1c] hover:bg-[#fef2f2] hover:text-[#b91c1c]",
					)}
					disabled={isPending}
				>
					{isPending ? (
						...
						<Users
							className={cn(
								"h-4 w-4",
								selectedUsers.length === 0 ? "text-[#b91c1c]" : "text-gray-400",
							)}
						/>
					)}
					{selectedUsers.length === 0 ? (
						<span className="text-sm">Assign PIC</span>
					) : (
